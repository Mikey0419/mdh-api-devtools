...
<div class="mt-6 grid gap-6 lg:grid-cols-[340px_1fr]"><div class="panel p-3"><div class="px-2 py-2 text-sm font-semibold">Incoming requests <span class="text-slate-500">({{events.length}})</span></div><div v-if="!events.length" class="p-6 text-center text-sm text-slate-500">Waiting for a request…</div><button v-for="e in events" :key="e.id" @click="selected=e" class="mb-2 w-full rounded-sm border border-white/5 bg-white/[.03] p-3 text-left hover:border-cyan-300/20 hover:bg-white/[.07]"><div class="flex justify-between"><span class="font-bold text-cyan-300">{{e.method}}</span><span class="text-xs text-slate-500">{{new Date(e.receivedAt).toLocaleTimeString()}}</span></div><div class="mt-1 truncate text-xs text-slate-400">{{e.path}}</div></button></div>
